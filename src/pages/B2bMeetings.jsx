...
import { Users, Plus, Upload, Building2, TrendingUp, Percent, CalendarCheck } from 'lucide-react';
...
export default function B2bMeetings() {
  ...
  const handleImport = async (e, mode) => {
    ...
    try {
      ...
      api.get(`/api/b2b/dashboard?client_company_id=${clientId}`).then(setDashboard);
      api.get(`/api/b2b/records?client_company_id=${clientId}`).then(setRecords);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <p className="text-brand-muted text-sm mb-6">Panel por cliente: bases contactadas y reuniones agendadas</p>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <select
          ...
          className="px-3 py-2 rounded-lg bg-brand-panel border border-brand-border text-sm font-tech"
        >
      ...
      {!clientId ? (
        <div className="text-center py-16 text-brand-muted text-sm border border-dashed border-brand-border rounded-xl">
          Agrega un cliente para empezar a cargar sus bases y reuniones.
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <label className="px-3 py-2 rounded-lg bg-brand-panel border border-brand-border text-xs cursor-pointer hover:border-brand-violet transition flex items-center gap-1.5">
                ...
                <input ref={contactedInputRef} type="file" accept=".csv" className="hidden" onChange={(e) => handleImport(e, 'contactados')} />
              </label>
              <label className="px-3 py-2 rounded-lg bg-gradient-to-r from-brand-violet to-brand-magenta text-xs cursor-pointer hover:opacity-90 transition flex items-center gap-1.5">
                <CalendarCheck size={13} /> Importar reuniones agendadas
                <input ref={meetingsInputRef} type="file" accept=".csv" className="hidden" onChange={(e) => handleImport(e, 'reuniones')} />
              </label>
            </div>
          </div>
          ...
          <div className="bg-brand-panel border border-brand-border rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {records.map((r) => (
                  <tr key={r.id} className="border-t border-brand-border hover:bg-brand-bg/50 transition">
                ...
                {records.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-brand-muted text-sm">Sin registros todavía. Importa una base de contactados para empezar.</td></tr>
                )}
              </tbody>
            </table>
          </div>
